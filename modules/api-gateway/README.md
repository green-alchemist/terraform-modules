...
| <a name="input_enable_lambda_fallback"></a> [enable\_lambda\_fallback](#input\_enable\_lambda\_fallback) | Enable the /scale-up route for manual Lambda trigger (only for HTTP\_PROXY). | `bool` | `false` | no |
...
| <a name="input_integration_type"></a> [integration\_type](#input\_integration\_type) | The integration type. Supported values: 'HTTP\_PROXY' (for ECS), 'AWS\_PROXY' (for Lambda). | `string` | n/a | yes |
| <a name="input_integration_uri"></a> [integration\_uri](#input\_integration\_uri) | The integration URI. For HTTP\_PROXY, this is the Cloud Map service ARN. For AWS\_PROXY, this is the Lambda ARN. | `string` | n/a | yes |
| <a name="input_lambda_fallback_arn"></a> [lambda\_fallback\_arn](#input\_lambda\_fallback\_arn) | ARN of the Lambda function for /scale-up route (only for HTTP\_PROXY). | `string` | `""` | no |
...
| <a name="input_route_keys"></a> [route\_keys](#input\_route\_keys) | A list of route keys to create for the integration (e.g., ['ANY /{proxy+}', 'POST /scale-up']). | `list(string)` | <pre>[<br/>  "ANY /{proxy+}"<br/>]</pre> | no |
