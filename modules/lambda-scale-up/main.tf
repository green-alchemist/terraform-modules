...
export const handler = async (event, context) => {
    ...
    log('INFO', 'Lambda invoked by Step Function', { requestId, action: event.action });
    ...
    try {
        switch (event.action) {
            ...
            case 'checkHealth':
                const target = await getHealthyInstance(process.env.CLOUD_MAP_SERVICE_ID, requestId);
                if (target) {
                    return { body: { status: 'READY', ip: target.ip, port: target.port }};
                } else {
            ...
            case 'proxy':
                ...
                return response; // Return the full HTTP response object
...
## old lambda code pre step function
# }
# locals {
#   lambda_code = <<-EOF
# import http from 'http';
