...
data "aws_iam_policy_document" "ecs_execution_ssm_policy" {
  ...
  statement {
    ...
    resources = [
      "arn:aws:ssm:${data.aws_region.current.name}:${data.aws_caller_identity.current.account_id}:parameter${var.secrets_ssm_path}",
      "arn:aws:secretsmanager:${data.aws_region.current.name}:${data.aws_caller_identity.current.account_id}:secret:*" # Adjust if needed
    ]
  }
}
